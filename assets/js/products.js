...
// Siempre muestra productos: primero renderiza una lista embebida (visible YA),
// luego intenta cargar assets/products.json (rompe caché) y reemplaza si sale bien.
...
(function () {
  ...
  // layout de grilla por si los estilos no cargan
  ...
  // Lista embebida (incluye el JBL). Esto se ve al instante.
  let base = [
    {
      "id": "jbl-510bt-negro",
  ...
  function render(lista) {
    ...
    lista.forEach(p => {
      const node = ($tpl ? $tpl.content : document.createElement('template').content).cloneNode(true) || (function(){
        ...
        art.innerHTML = `<div style="position:relative;aspect-ratio:1/1;background:#0d1b2a;">
        ...
        const tpl = document.createElement('template'); tpl.content.appendChild(art); return tpl.content.cloneNode(true);
      })();
  ...
  // Primera vista: mostramos la embebida (ya se ve TODO, incluido el JBL)
  ...
  // Ahora intentamos cargar el JSON real y reemplazar 'base' si sale bien
  fetch(URL_JSON, { cache: 'no-store' })
    .then(r => (r.ok ? r.json() : Promise.reject(new Error('HTTP ' + r.status))))
    .then(data => {
      if (!Array.isArray(data)) throw new Error('JSON no es array');
      base = data; // ← a partir de acá los filtros actúan sobre el JSON real
      // Reseteo filtros y muestro todo
      if ($buscar) $buscar.value = '';
      if ($filtro) $filtro.value = '';
      render(base);
    })
    .catch(err => {
      console.warn('[catalogo] No se pudo leer products.json, se queda la lista embebida:', err);
      // Nos quedamos con 'base' embebida, ya visible.
    });
})();
